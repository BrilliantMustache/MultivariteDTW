...
from SourceCode.X3  import findBoundingBoxes
import time
...
def getLB_oneQR_boxR (Q, R, bboxes):
    '''
...
def DTWDistanceWindowLB_Ordered_Xs3 (queryID, X0LBs, DTWdist, bboxes, K, Q, s1, refs, W, TH=1):
    '''
    ...
    for x in range(1, len(LBSortedIndex)):
        if X0LBs[LBSortedIndex[x]] > dist:
            ...
            c_lb = getLB_oneQR_boxR(s1, refs[LBSortedIndex[x]], bboxes[LBSortedIndex[x]])
            cluster_cals += 1
            if c_lb <= dist:
        ...
        for w in windows:
            ...
            for K in Ks:
                for Q in Qs:
                    ...
                    results = [DTWDistanceWindowLB_Ordered_Xs3 (ids1, lb2003[ids1], distances, bboxes, K, Q,
                    ...
                    with open(toppath + str(nqueries) + "X" + str(
                            nreferences) + "_Xs3K" + str(K) + "Q" + str(Q) + "_results.txt", 'w') as f:
                        for r in results:
                            f.write(str(r) + '\n')
                    allsetupTimes.append(setuptime)
    np.save(pathUCRResult + '_AllDataSets/' + 'd' + str(maxdim) + "/" + str(nqueries) + "X" + str(nreferences)
            + "_Xs3w" + intlist2str(windows) +"K" + intlist2str(Ks)+ "Q" + intlist2str(Qs) + "_setuptimes.npy", allsetupTimes)
    return 0
...
def dataProcessing(datasetsNameFile, pathUCRResult="../Results/UCR/", maxdim = 5, nqueries = 3, nreferences = 20, windows = [20], Ks=[6], Qs=[2],machineRatios=[1,1]):
            ...
            for Q in Qs:
                results = readResultFile(
                    pathUCRResult + dataset + '/d' + str(maxdim) + "/w" + str(windows[0]) + "/" + str(nqueries) + "X" + str(
                        nreferences) + "_Xs3K" + str(K) + "Q"+str(Q)+"_results.txt")
    ...
    np.save(pathUCRResult + "_AllDataSets/" + 'd' + str(maxdim) + '/' + str(nqueries) + "X" + str(nreferences) +
            "_Xs3w" + str(window) + "K" + intlist2str(Ks) + "Q" + intlist2str(Qs) + '_speedups.npy', speedups)
    np.save(pathUCRResult + "_AllDataSets/" + 'd' + str(maxdim) + '/' + str(nqueries) + "X" + str(nreferences) +
            "_Xs3w" + str(window) + "K" + intlist2str(Ks) + "Q" + intlist2str(Qs) + '_skipschosen.npy', skips_chosen)
    np.save(pathUCRResult + "_AllDataSets/" + 'd' + str(maxdim) + '/' + str(nqueries) + "X" + str(nreferences) +
            "_Xs3w" + str(window) + "K" + intlist2str(Ks) + "Q" + intlist2str(Qs) + '_settingchosen.npy', setting_chosen)
    np.save(pathUCRResult + "_AllDataSets/" + 'd' + str(maxdim) + '/' + str(nqueries) + "X" + str(nreferences) +
            "_Xs3w" + str(window) + "K" + intlist2str(Ks) + "Q" + intlist2str(Qs) + '_overheadrate.npy', overheadrate)
