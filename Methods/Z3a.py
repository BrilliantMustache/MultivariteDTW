...
#    start = time.time()
    for x in range(1, len(LBSortedIndex)):
        ...
        if M0LBs[thisrefid] >= dist:
            ...
            dist2 = DTW_a(query, references, W, dist)
            if dist > dist2:
                dist = dist2
            ...
            for K in Ks:
                for Q in Qs:
                    ...
                    with open(toppath + str(nqueries) + "X" + str(
                            nreferences) + "_Z3_a_K" + str(K) + "Q" + str(Q) + "_results.txt", 'w') as f:
                        for r in results:
                            f.write(str(r) + '\n')
            ...
            for Q in Qs:
                results = readResultFile(
                    pathUCRResult + dataset + '/d' + str(maxdim) + "/w" + str(windows[0]) + "/" + str(nqueries) + "X" + str(
                        nreferences) + "_Z3_a_K" + str(K) + "Q"+str(Q)+"_results.txt")
...
#    overheadrate = overhead/(rdtw * t1dtw * NPairs)
    ...
    np.save(pathUCRResult + "_AllDataSets/" + 'd' + str(maxdim) + '/' + str(nqueries) + "X" + str(nreferences) +
            "_Z3_a_w" + str(window) + "K" + intlist2str(Ks) + "Q" + intlist2str(Qs) + '_speedups.npy', speedups)
    np.save(pathUCRResult + "_AllDataSets/" + 'd' + str(maxdim) + '/' + str(nqueries) + "X" + str(nreferences) +
            "_Z3_a_w" + str(window) + "K" + intlist2str(Ks) + "Q" + intlist2str(Qs) + '_skipschosen.npy', skips_chosen)
    np.save(pathUCRResult + "_AllDataSets/" + 'd' + str(maxdim) + '/' + str(nqueries) + "X" + str(nreferences) +
            "_Z3_a_w" + str(window) + "K" + intlist2str(Ks) + "Q" + intlist2str(Qs) + '_settingchosen.npy', setting_chosen)
#    np.save(pathUCRResult + "_AllDataSets/" + 'd' + str(maxdim) + '/' + str(nqueries) + "X" + str(nreferences) +
#            "_Z3_a_w" + str(window) + "K" + intlist2str(Ks) + "Q" + intlist2str(Qs) + '_overheadrate.npy', overheadrate)
