...
#    datasets=["ArticularyWordRecognition","AtrialFibrillation"]
    ...
    speedups = (rdtw*t1dtw[0:ndatasets]*NPairs)/(rother*tCore+rdtw*tDTW)
    overheadrate = (rother*tCore)/(rdtw*t1dtw[0:ndatasets]*NPairs)

    np.save(pathUCRResult+"_AllDataSets/" + 'd' + str(maxdim) + '/' + str(nqueries) + "X" + str(nreferences) +
            "_Z0_w"+str(window)+'_speedups.npy', speedups)
    np.save(pathUCRResult + "_AllDataSets/" + 'd' + str(maxdim) + '/' + str(nqueries) + "X" + str(nreferences) +
            "_Z0_w" + str(window) + '_skips.npy', skips)
    np.save(pathUCRResult + "_AllDataSets/" + 'd' + str(maxdim) + '/' + str(nqueries) + "X" + str(nreferences) +
            "_Z0_w" + str(window) + '_overheadrate.npy', overheadrate)
    return 0
