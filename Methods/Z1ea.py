...
from Methods.X1e import *

# This file implements Z1 with early abandoning

def DTWDistanceWindowLB_Ordered_Z1_ea_ (queryID, DTWdist, TH, P, query, references, W):
    '''
...
#    start = time.time()
    for x in range(1, len(LBSortedIndex)):
        if LBs[LBSortedIndex[x]] > dist:
            ...
            p_lb = tiBounds_top_calP_list_comp_eb(query, references[LBSortedIndex[x]], P, W, dxx, dist)
            p_cals += 1
            if p_lb <= dist:
...
#            dists = [[DTW(s1, s2, windowSize) for s2 in reference] for s1 in query]
            for TH in THs:
                results = [DTWDistanceWindowLB_Ordered_Z1_ea_ (ids1, distances, TH,
                ...
                with open(toppath+ str(nqueries) + "X" + str(
                    nreferences) + "_Z1_ea_TH"+str(TH)+"_results.txt", 'w') as f:
                    for r in results:
                        f.write(str(r)+'\n')
                f.close()
                # with open(toppath+ str(nqueries) + "X" + str(
    ...
    for dataset in datasets:
        for TH in THs:
            ...
            pathUCRResult + dataset + '/d' + str(maxdim) + "/w" + str(windows[0]) + "/" + str(nqueries) + "X" + str(
                nreferences) + "_Z1_ea_TH" + str(TH) + "_results.txt")
...
#    overheadrate = overhead/(rdtw*t1dtw * NPairs)
    ...
    np.save(pathUCRResult + "_AllDataSets/" + 'd' + str(maxdim) + '/' + str(nqueries) + "X" + str(nreferences) +
            "_Z1_ea_w" + str(window) + 'TH'+intlist2str(THs)+'_speedups.npy', speedups)
    np.save(pathUCRResult + "_AllDataSets/" + 'd' + str(maxdim) + '/' + str(nqueries) + "X" + str(nreferences) +
            "_Z1_ea_w" + str(window) + 'TH' + intlist2str(THs) + '_skipschosen.npy', skips_chosen)
    np.save(pathUCRResult + "_AllDataSets/" + 'd' + str(maxdim) + '/' + str(nqueries) + "X" + str(nreferences) +
            "_Z1_ea_w" + str(window) + 'TH' + intlist2str(THs) + '_settingchosen.npy', setting_chosen)
#    np.save(pathUCRResult + "_AllDataSets/" + 'd' + str(maxdim) + '/' + str(nqueries) + "X" + str(nreferences) +
#            "_Z1_ea_w" + str(window) + 'TH' + intlist2str(THs) + '_overheadrate.npy', overheadrate)
    return 0
