...
def DTWDistanceWindowLB_Ordered_X3rse_ (queryID, X0LBs, DTWdist, bboxes, K, Q, s1, refs, W, TH=1):
    '''
        ...
        for w in windows:
            ...
            for K in Ks:
                for Q in Qs:
                    ...
                    results = [DTWDistanceWindowLB_Ordered_X3rse_ (ids1, lb2003[ids1], distances, bboxes, K, Q,
            ...
            for Q in Qs:
                results = readResultFile(
                    pathUCRResult + dataset + '/d' + str(maxdim) + "/w" + str(windows[0]) + "/" + str(nqueries) + "X" + str(
                        nreferences) + "_X3rse_K" + str(K) + "Q"+str(Q)+"_results.txt")
