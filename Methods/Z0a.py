...
#                    + str(nqueries) + "X" + str(nreferences) + "_Z0_a_lbs.npy", np.array(LBs_g))
            with open(toppath + str(nqueries) + "X" + str(
                    nreferences) + "_Z0_a" + "_results.txt", 'w') as f:
                for r in results:
                    f.write(str(r) + '\n')
            f.close()
...
#    overheadrate = (rother*tCore)/(rdtw*t1dtw*NPairs)
    ...
    np.save(pathUCRResult+"_AllDataSets/" + 'd' + str(maxdim) + '/' + str(nqueries) + "X" + str(nreferences) +
            "_Z0_a_w"+str(window)+'_speedups.npy', speedups)
    np.save(pathUCRResult + "_AllDataSets/" + 'd' + str(maxdim) + '/' + str(nqueries) + "X" + str(nreferences) +
            "_Z0_a_w" + str(window) + '_skips.npy', skips)
#    np.save(pathUCRResult + "_AllDataSets/" + 'd' + str(maxdim) + '/' + str(nqueries) + "X" + str(nreferences) +
#            "_Z0_a_w" + str(window) + '_overheadrate.npy', overheadrate)
    return 0
