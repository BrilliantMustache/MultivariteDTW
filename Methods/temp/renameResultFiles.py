...
def renameOrder (oldpattern, newpattern):
    for dataset in datasets_g:
        datasetpath = pathUCRResults_g + dataset + '/'
        dirs = [x[0] for x in os.walk(datasetpath)]
        for d in dirs[1:]:
...
#renameOrder('X2_0', 'X20')
#renameOrder('X1_e_', 'X1e_')
# renameOrder('X3_r_', 'X3r_')
# renameOrder('X3_rse', 'X3rse_')
renameOrder('x3s_', 'X3s_')
# renameOrder('X3_z', 'X3z_')
# renameOrder('Z1_e','Z1e_')
# renameOrder('Z3_e', 'Z3e_')
