...
args = len(argvs)
if (args < 3):
    print('Please indicate which datasets and which result path to work on.')
    exit()
datasetsIDs = argvs[1].split(',')
datasetsIDs = [int(x) for x in datasetsIDs]
pathUCRResult = argvs[2] + '/'
...
datasetsNameFile = pathUCRResult + "datasets_node"+hostname+"_stage3.txt"
datasetsSizeFile = pathUCRResult + "datasets_size_node"+hostname+"_stage3.txt"
...
with open(logfile, 'a+') as f:
    f.write('Node ' + hostname + ' : ' + ",".join(alldatasets) + " . Stage 3.")
